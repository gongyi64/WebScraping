...
layout =[[sg.Text('[NT勤務自動ダウンロード]',font = ('Noto Serif CJK JP',14))],
        ...
        [sg.Text('[入力ボタンを押して確認後、実行ボタンを押して開始。] ',font = ('meiryo',10))],
        [sg.Button('入力',button_color=('yellow','#808080'),key= '-INPUT-'),sg.Text('【勤務年月】', text_color = 'white',key = '-text1-')],
...
# for i in range(1):  # 全員分（出向、スタッフ除く）の勤務取得ルーチン。
...
# df_kinmu = pd.DataFrame()
    ...
    for i in range(start,len(trs)):#経営職では、1からにしていた。後日要チェック
...
# ''.join(df_org[0].splitlines())#Seriesではだめ\nを消したい
    df_org.drop(index = [1],inplace = True)

    print('余計な行削除')

    print(df_org)
#0,2行削除する！  print(type(df_org))#Dataframe
...
#結合セルによるデータずれの補正(1行目）。2行目と統合させ1行に変更。ひとつひとつやるしかなさそう。
        ...
        # if os.path.exists('c:/Users/406239/OneDrive - (株)NHKテクノロジーズ/デスクトップ/★勤務確認などのダウンロードデータ★/検証中/{num}_smart_kinmu.xlsx') == False:
        #     wb = openpyxl.Workbook()
        #     path = fr'c:/Users/406239/OneDrive - (株)NHKテクノロジーズ/デスクトップ/★勤務確認などのダウンロードデータ★/検証中/{num}_smart_kinmu.xlsx'
        #     wb.save(path)
        # else:
        #     continue

        with pd.ExcelWriter(f'c:/Users/406239/OneDrive - (株)NHKテクノロジーズ/デスクトップ/★勤務確認などのダウンロードデータ★/検証中/{num}_smart_kinmu.xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
                df_org.to_excel(writer, sheet_name= f'{num}_{name}')

        # elif os.path.exists('c:/Users/406239/OneDrive - (株)NHKテクノロジーズ/デスクトップ/★勤務確認などのダウンロードデータ★/検証中/{num}_smart_kinmu.xlsx') == True:
        #     with pd.ExcelWriter(f'c:/Users/406239/OneDrive - (株)NHKテクノロジーズ/デスクトップ/★勤務確認などのダウンロードデータ★/検証中/{num}_smart_kinmu.xlsx', engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
        ...
        #
        # driver.maximize_window()
        # driver.execute_script('window.scrollTo(0, document.body.scrollHeight);')#最下部までスクロールさせる
        #
    ...
    # 0行目をcolumnにするコード　（以下）
        ...
        df_org = df_org.drop(df_org.index[0])
        df_org = df_org.drop(columns=df_org.columns[1])
    ...
    # set input file name
    inputfile = f'c:/Users/406239/OneDrive - (株)NHKテクノロジーズ/デスクトップ/★勤務確認などのダウンロードデータ★/検証中/{num}_smart_kinmu.xlsx'
    ...
    time.sleep(2)
    if val_s == '0':
        driver.find_element(By.CSS_SELECTOR,'body > table:nth-child(4) > tbody > tr:nth-child(4) > td:nth-child(4) > input').click()  # 終了押す
    elif val_s == '1':
        driver.find_element(By.CSS_SELECTOR,'body > form > table > tbody > tr:nth-child(2) > td:nth-child(4) > input').click()
    else:
        continue
